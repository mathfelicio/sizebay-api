...
describe('ShortUrlPostgresRepository', () => {
  ...
  let typeOrmRepository: jest.Mocked<Repository<ShortUrlEntity>>;
  ...
  beforeEach(async () => {
    ...
    repository = module.get<ShortUrlPostgresRepository>(ShortUrlPostgresRepository);
    typeOrmRepository = module.get(getRepositoryToken(ShortUrlEntity));
  });
  ...
  it('should find by code', async () => {
    ...
    expect(typeOrmRepository.findOne).toHaveBeenCalledWith({ where: { code: 'code123' } });
  ...
  it('should find by originalUrl', async () => {
    ...
    expect(typeOrmRepository.findOne).toHaveBeenCalledWith({ where: { originalUrl: 'https://example.com' } });
    expect(result).toBeDefined();
